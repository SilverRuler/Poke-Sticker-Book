...
function App() {
  ...
  const [anniversaryCollection, setAnniversaryCollection] = useState<CollectionMap>({});
  const [pendingAnniversaryCollection, setPendingAnniversaryCollection] = useState<CollectionMap>({});
  ...
  const fetchCollections = async () => {
    try {
      ...
      const migrateAnniv = (val: any) => {
        if (Array.isArray(val)) {
          const obj: CollectionMap = {};
          val.forEach(k => { obj[k] = 1; });
          return obj;
        }
        return val || {};
      };

      const localAnniv = migrateAnniv(JSON.parse(localStorage.getItem("anniversary_collection") || "[]"));
      const localPendingAnniv = migrateAnniv(JSON.parse(localStorage.getItem("pending_anniversary_collection") || "[]"));
  ...
  const toggleAnniversary = async (key: string, isPending: boolean, anniversaryCount?: number) => {
    try {
      ...
      const collKey = isPending ? "pending_collection" : "collection";
      const localAnniv = JSON.parse(localStorage.getItem(storageKey) || "{}");
      const localColl = JSON.parse(localStorage.getItem(collKey) || "{}");
      
      const totalCount = localColl[key] || 0;
      if (totalCount === 0) return;

      const updatedAnniv = { ...localAnniv };
      if (anniversaryCount !== undefined) {
        const newCount = Math.max(0, Math.min(anniversaryCount, totalCount));
        if (newCount === 0) delete updatedAnniv[key];
        else updatedAnniv[key] = newCount;
      } else {
        if (updatedAnniv[key]) {
          delete updatedAnniv[key];
        } else {
          updatedAnniv[key] = 1;
        }
      }
    ...
    return (
      <div className="pokemon-grid">
        {keys.map((key) => {
          ...
          const annivCount = (isPending ? pendingAnniversaryCollection[key] : anniversaryCollection[key]) || 0;
          const isAnniversary = annivCount > 0;
          return (
    ...
    <div className="app-container">
                ...
                <div className="detail-layout">
                  <div className="detail-left">
                        ...
                        <div className="anniversary-ui-overlay">
                          ...
                          {(activeTab === "pending" ? pendingAnniversaryCollection[detailKey] : anniversaryCollection[detailKey]) > 0 && (
                            <div className="anniversary-count-controls">
                              ...
                              <span className="anniv-count-display">
                                {(activeTab === "pending" ? pendingAnniversaryCollection[detailKey] : anniversaryCollection[detailKey])}
                              </span>
                              <button 
                                className="btn-count"
                                onClick={(e) => {
                    ...
                    <div className="detail-form-info">
                      <div style={{ fontSize: "0.9rem", color: "#888", marginBottom: "2px" }}>
                        No.{detailKey.split("-")[0].padStart(4, "0")}
                      </div>
                      <div>{getPokemonByKey(detailKey)?.name}</div>
                      {activeTab === "pending" && isLoggedIn && (
                        <div style={{ marginTop: "10px" }}>
                          <button 
                            className="btn btn-mini btn-primary btn-today-action"
      ...
      <footer className="footer">
        ...
        <p className="footer-link">
          <a href="https://github.com/SilverRuler/Poke-Sticker-Book" target="_blank" rel="noopener noreferrer" className="github-link">
            <svg className="github-icon" viewBox="0 0 19 19" width="16" height="16">
